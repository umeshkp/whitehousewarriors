...
#### Scenario: Verified Gmail profile enables authenticated mode
- **WHEN** profile extraction succeeds and normalized email ends with `@gmail.com`
- **THEN** the system allows authenticated-mode transition and downstream sheet-enabled flow

#### Scenario: Non-Gmail profile blocks authenticated mode
- **WHEN** profile extraction succeeds but normalized email does not end with `@gmail.com`
- **THEN** the system rejects authenticated-mode transition and surfaces a Gmail-required message
